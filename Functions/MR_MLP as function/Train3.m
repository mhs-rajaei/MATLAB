...
% % Targets
% labels = feature_target;
% 
% tlabels = t_feature_target;
% 
% timages = t_inputs;
% 
% [t_size,~] =  size(t_inputs);

for epoch=1:iteration % forward and update weight's in number of  iterations
    delta_W=zeros();
    delta_theta=zeros();
    for num_in=1:number_of_training_samples
%         if layer(1).a(:,number_of_training_samples)==0
%             continue;
%         end
        %% =========== Forward =============
        counter = counter +1;
    ...
    Accuracy_Train(epoch) = Test(layer,number_of_training_samples,Train_or_Test,L,labels,trainig_inputs,tanh_or_sigmoid)/number_of_training_samples;
    %
    ...
    %
    % Compute Overal MSE
    MSE(epoch) = (sum(layer(L).MSE(:))^0.5) / number_of_training_samples;
