...
M = 400; %set the number of hidden layer neurons
...
for ii = 1:NumIterations
    dW = ((A-A.^2).*(W_outputs'*(W_outputs*A-Y')))*X'; %assumes logistic function sigmoidal activations
    W_randoms = W_randoms - Epsilon*dW; %get new W_in
    A = 1./(1+exp(-W_randoms*X)); %get new A
    W_outputs = (A*Y)'/(A*A'+RidgeParam*eye(M)); %get new W_out
    
    Y_predicted_test = W_outputs*(1./(1+exp(-W_randoms*X_test))); %get output layer response
    [MaxVal,ClassificationID_test] = max(Y_predicted_test); %get classification
    PercentCorrect = 100*(1-length(find(ClassificationID_test-1-labels_test'~=0))/length(labels_test))
end
