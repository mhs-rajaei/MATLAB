...
opts.continue = false ;
opts.expDir = fullfile('F:\Documents\MATLAB\Machine Learning\MR  ML Project\CNN MNIST','exp') ;%export directory
...
for epoch=start+1:opts.numEpochs
  ...
  val = opts.val ;
  if numGpus <= 1
    [net,stats.train] = process_epoch(opts, getBatch, epoch, train,...
        learningRate, imdb, net) ;
    [~,stats.val] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net) ;
  else
    spmd(numGpus)
      [net_, stats_train_] = process_epoch(opts, getBatch, epoch, train,...
          learningRate, imdb, net) ;
...
% -------------------------------------------------------------------------
function  [net_cpu,stats,prof] = process_epoch(opts, getBatch, epoch,...
    subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------
